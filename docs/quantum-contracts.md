...
These are function code annotations that specify pre- & post-conditions that the quantum data should satisfy. This is an attempt track entanglement at the level of type system, and obviously it can only be achieved only within some limits, otherwise we would not need a quantum computer at all. When a condition cannot be checked statically a Lean proof obligation will be generated by the compiler:
